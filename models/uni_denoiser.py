...
from models.common import GaussianSmearing, MLP, AngularEncoding, batch_hybrid_edge_connection, outer_product, \
        get_direction_feat


class NodeUpdateLayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_heads, edge_feat_dim,
                 act_fn='relu', norm=True, out_fc=True, direction_match=False):
        super().__init__()
        ...
        kv_input_dim = kv_input_dim + 9 if direction_match else kv_input_dim
...
class PosUpdateLayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_heads, edge_feat_dim,
                 act_fn='relu', norm=True, direction_match=False):
        super().__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim
...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    def __init__(self, hidden_dim, n_heads, num_r_gaussian, edge_feat_dim, act_fn='relu', norm=True,
                 r_min=0., r_max=10., include_h_node=False, x2h_out_fc=True, direction_match=False):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.n_heads = n_heads
        self.edge_feat_dim = edge_feat_dim
        ...
        self.direction_match = direction_match
        ...
        self.pos_layer_with_edge = PosUpdateLayer(
            hidden_dim, hidden_dim, hidden_dim, n_heads,
            edge_feat_dim=num_r_gaussian * edge_feat_dim + edge_feat_dim,
            act_fn=act_fn, norm=norm, direction_match=direction_match
        )
        ...
        if self.direction_match:
            self.dire_embedding = nn.Linear(3, 9)

    def forward(self, h, x, edge_attr, edge_index, h_bond, bond_index, mask_ligand, e_w=None, 
                phore_norm=None, batch=None):

        # -> phore-ligand knn graph message passing
        src, dst = edge_index
        rel_x = x[dst] - x[src]
        ...
        if self.direction_match:
            dire_feat = get_direction_feat(
                x=x, phore_norm=phore_norm, 
                edge_index=edge_index, 
                mask_ligand=mask_ligand, 
                batch=batch
            )
            dire_feat = self.dire_embedding(dire_feat)
            edge_feat = torch.cat([edge_feat, dire_feat], dim=-1)
...
class UniTransformerO2TwoUpdateGeneralBond(nn.Module):
    def __init__(self, num_blocks, num_layers, hidden_dim, n_heads=1, k=32, num_bond_classes=1,
                 ...
                 r_max=10., x2h_out_fc=True, h_node_in_bond_net=False, direction_match=False):
        super().__init__()
        # Build the network
    ...
    def _build_share_blocks(self):
        ...
        for l_idx in range(self.num_layers):
            layer = AttentionLayerO2TwoUpdateNodeGeneral(
                ...
                r_max=self.r_max,
                x2h_out_fc=self.x2h_out_fc, include_h_node=self.h_node_in_bond_net, direction_match=self.direction_match
            )
    ...
    def _build_edge_type(self, edge_index, mask_ligand, decomp_group_idx):
        ...
        if decomp_group_idx is not None:
            # denote whether phore / ligand / dummy are from the same prior
    ...
    def forward(self, h, x, group_idx, bond_index, h_bond, mask_ligand, mask_ligand_atom, batch, 
                phore_norm=None, return_all=False):
        all_x = [x]
        all_h = [h]
        all_h_bond = [h_bond]

        for b_idx in range(self.num_blocks):
            # edge index for (p-l, l-p, l-l, p-p) knn graph
            edge_index = self._connect_edge(x, mask_ligand, batch)

            # edge type (dim: 4) for (p-l, l-p, l-l, p-p) knn graph
            ...
            for l_idx, layer in enumerate(self.base_block):
                h, h_bond, x = layer(h, x, edge_type, edge_index, h_bond, bond_index, mask_ligand_atom, e_w=e_w, 
                                     phore_norm=phore_norm, batch=batch)
            all_x.append(x)
            all_h.append(h)
